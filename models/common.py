...
def get_r_feat(r, r_exp_func, node_type=None, edge_index=None, mode='basic'):
 ...
 # how to add pos_motif batch_motif to  h_ctx, pos_ctx, batch_ctx?
def compose_context(h_protein, h_ligand, pos_protein, pos_ligand, batch_protein, batch_ligand, is_in_pocket, h_motif, pos_motif, batch_motif):
    # previous version has problems when ligand atom types are fixed
    # (due to sorting randomly in case of same element)

    batch_ctx = torch.cat([batch_protein, batch_ligand, batch_motif], dim=0)
    ...
    mask_protein = torch.cat([
        ...
        torch.zeros([batch_motif.size(0)], device=batch_motif.device).bool(),
    ], dim=0)[sort_idx]
    ...
    mask_motif = torch.cat([
        torch.zeros([batch_protein.size(0)], device=batch_protein.device).bool(),
        torch.zeros([batch_ligand.size(0)], device=batch_ligand.device).bool(),
        torch.ones([batch_motif.size(0)], device=batch_motif.device).bool(),
    ], dim=0)[sort_idx]

    batch_ctx = batch_ctx[sort_idx]
    h_ctx = torch.cat([h_protein, h_ligand, h_motif], dim=0)[sort_idx]  # (N_protein+N_ligand, H)
    pos_ctx = torch.cat([pos_protein, pos_ligand, pos_motif], dim=0)[sort_idx]  # (N_protein+N_ligand, 3)

    return h_ctx, pos_ctx, batch_ctx, mask_ligand, mask_pocket, mask_motif, mask_protein
