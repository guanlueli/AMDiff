...
class hier_diff(nn.Module):
    ...
    def forward(self, protein_pos, protein_v, protein_ph, batch_protein, batch_is_in_pocket, init_ligand_pos, init_ligand_v, ligand_ph, batch_ligand,
        ...
        if self.time_emb_dim > 0:
            if self.time_emb_mode == 'simple':
                if self.config.ligand_ph_feature:
                    ligand_ph_batch = ligand_ph[batch_ligand]
                    ligand_ph_batch = self.ligand_ph_emb(ligand_ph_batch)
                    input_ligand_feat = torch.cat([init_ligand_v, ligand_ph_batch], -1)
                    input_ligand_feat = torch.cat([input_ligand_feat, (time_step / self.num_timesteps)[batch_ligand].unsqueeze(-1)], -1)

                    input_motif_feat = torch.cat(
                        [init_motif_v, (time_step / self.num_timesteps)[batch_motif].unsqueeze(-1)], -1)
                else:
    ...
    def get_diffusion_loss(
        ...
        # Xt = a.sqrt() * X0 + (1-a).sqrt() * eps
        motif_pos_perturbed = a_pos_motif.sqrt() * motif_pos + (1.0 - a_pos_motif).sqrt() * pos_noise_motif  # pos_noise * std
        # motif_pos_perturbed =  motif_pos   # pos_noise * std
        log_motif_v0 = index_to_log_onehot(motif_wid, self.num_classes_motif)
        motif_wid_perturbed, log_motif_vt = self.q_v_sample(log_motif_v0, time_step, batch_motif, self.num_classes_motif)
        # motif_wid_perturbed = log_motif_v0
        ...
        preds = self(
            ...
            init_motif_wid = motif_wid_perturbed,
            init_motif_pos = motif_pos_perturbed,
            batch_motif = batch_motif,
        ...
        loss = loss_pos + loss_v * self.loss_v_weight + loss_pos_motif * 0.01 + loss_v_motif * 100
