...
class XtoXAttLayer(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, n_heads, edge_feat_dim, r_feat_dim,
        ...
        kv_input_dim = edge_feat_dim + r_feat_dim

        self.xk_func = MLP(kv_input_dim, output_dim, hidden_dim, norm=norm, act_fn=act_fn)
        self.xv_func = MLP(kv_input_dim, self.n_heads, hidden_dim, norm=norm, act_fn=act_fn)
        self.xq_func = MLP(3, output_dim, hidden_dim, norm=norm, act_fn=act_fn)
        if ew_net_type == 'r':
            self.ew_net = nn.Sequential(nn.Linear(r_feat_dim, 1), nn.Sigmoid())
    ...
    def forward(self, h, rel_x, r_feat, edge_feat, edge_index, e_w=None):
        ...
        v = v.unsqueeze(-1) * rel_x.unsqueeze(1)  # (xi - xj) [n_edges, n_heads, 3]
        q = self.xq_func(rel_x).view(-1, self.n_heads, self.output_dim // self.n_heads)
...
class AttLayerUpdateNode(nn.Module):
    def __init__(self, hidden_dim, n_heads, num_r_gaussian, edge_feat_dim, act_fn='relu', norm=True,
                 ...
                 ew_net_type='r', x2h_out_fc=True, sync_twoup=False, x_to_x = False):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.n_heads = n_heads
        self.edge_feat_dim = edge_feat_dim
        ...
        for i in range(self.num_x2h):
            self.x2h_layers.append(
                XtoHAttLayer(hidden_dim, hidden_dim, hidden_dim, n_heads, edge_feat_dim,
                                r_feat_dim=num_r_gaussian * 8,
        ...
        for i in range(self.num_h2x):
            self.h2x_layers.append(
                HtoXAttLayer(hidden_dim, hidden_dim, hidden_dim, n_heads, edge_feat_dim,
                                r_feat_dim=num_r_gaussian * 8,
                                act_fn=act_fn, norm=norm,
        ...
        ## todo X to X
        if self.x_to_x:
            self.x2x_layers = nn.ModuleList()
            for i in range(self.num_x2x):
                self.x2x_layers.append(
                    XtoXAttLayer(hidden_dim, hidden_dim, hidden_dim, n_heads, edge_feat_dim,
                                    r_feat_dim=num_r_gaussian * 8,
                                    act_fn=act_fn, norm=norm,
                                    ew_net_type=self.ew_net_type)
                )

    def forward(self, h, x, edge_attr, edge_index, mask_ligand, mask_motif, e_w=None, fix_x=False):
        src, dst = edge_index
        ...
        for i in range(self.num_h2x):
            ...
            if not fix_x:
                x = x + delta_x * mask_ligand[:, None]  # only ligand and motif positions will be updated
                x = x + delta_x * mask_motif[:, None]
            rel_x = x[dst] - x[src]
            dist = torch.norm(rel_x, p=2, dim=-1, keepdim=True)
        ...
        if self.x_to_x:
            for i in range(self.num_x2x):
                dist_feat = self.distance_expansion(dist)
                dist_feat = outer_product(edge_attr, dist_feat)
                delta_x = self.x2x_layers[i](new_h, rel_x, dist_feat, edge_feat, edge_index, e_w=e_w)
                x = x + delta_x * mask_ligand[:, None] # only ligand and motif positions will be updated
                x = x + delta_x * mask_motif[:, None]
                rel_x = x[dst] - x[src]
                dist = torch.norm(rel_x, p=2, dim=-1, keepdim=True)
...
class ATT_EGNN(nn.Module):
    def __init__(self, num_blocks, num_layers, hidden_dim, n_heads=1, k=32,
                 ...
                 num_init_x2h=1, num_init_h2x=0, num_x2h=1, num_h2x=1, r_max=10., x2h_out_fc=True, sync_twoup=False, x_to_x = False):
        super().__init__()
        # Build the network
    ...
    def _build_init_h_layer(self):
        layer = AttLayerUpdateNode(
            ...
            ew_net_type=self.ew_net_type, x2h_out_fc=self.x2h_out_fc, sync_twoup=self.sync_twoup,x_to_x = self.x_to_x
        )
        return layer
    ...
    @staticmethod
    def _build_edge_type(edge_index, mask_ligand, mask_pocket, mask_motif, mask_protein):
        src, dst = edge_index
        edge_type = torch.zeros(len(src)).to(edge_index)
        l_src = mask_ligand[src] == 1
        l_dst = mask_ligand[dst] == 1

        po_src = mask_pocket[src] == 1
        po_dst = mask_pocket[dst] == 1

        f_src = mask_motif[src] == 1
        f_dst = mask_motif[dst] == 1

        p_src = mask_protein[src] == 1
        p_dst = mask_protein[dst] == 1

        edge_type[l_src & l_dst] = 0  # l l
        edge_type[l_src & p_dst] = 1 # l p
        edge_type[p_src & l_dst] = 2 # p l
        edge_type[p_src & p_dst] = 3 # p p

        # pocket
        edge_type[l_src & po_dst] = 4  # l po
        edge_type[po_src & l_dst] = 5  # po l

        # motif
        edge_type[l_src & f_dst] = 6  # l f
        edge_type[f_src & l_dst] = 7  # f l

        edge_type = F.one_hot(edge_type, num_classes=8)
        return edge_type

    def forward(self, h, x, mask_ligand, mask_pocket, mask_motif, mask_protein, batch, return_all=False, fix_x=False):
        ...
        for b_idx in range(self.num_blocks):
            ...
            # edge type (dim: 4)
            edge_type = self._build_edge_type(edge_index , mask_ligand, mask_pocket, mask_motif, mask_protein)
            if self.ew_net_type == 'global':
            ...
            for l_idx, layer in enumerate(self.base_block):
                h, x = layer(h, x, edge_type, edge_index, mask_ligand, mask_motif, e_w=e_w, fix_x=fix_x)
            all_x.append(x)
            all_h.append(h)
