...
def pdbqt_to_sdf(pdbqt_file='.pdbqt', output='.sdf', form = 'sdf'):
    # Read the PDBQT file
    mol = next(pybel.readfile('pdbqt', pdbqt_file))

    # Save the molecule as an SDF file
    mol.write(form, output, overwrite=True)
...
class PrepProt(object):
    ...
    def addH(self, prot_pqr):  # call pdb2pqr
        ...
        subprocess.Popen([pdb2pqr30_path, self.prot, self.prot_pqr],
    ...
    def get_pdbqt(self, prot_pdbqt):
        ...
        subprocess.Popen([python_executable, prepare_receptor, '-r', self.prot_pqr, '-o', prot_pdbqt],
                         stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL).communicate()
    ...
    def dock(self, score_func='vina', seed=0, mode='dock', exhaustiveness=8, save_pose=False, **kwargs):  # seed=0 mean random seed
        ...
        if not save_pose:
            ...
            if mode == 'score_only':
                ...
                self.lig_pdbqt_out = self.lig_pdbqt.replace('.pdbqt', '_out.pdbqt')
                v.write_poses(self.lig_pdbqt_out, n_poses=10, overwrite=True)

            else:
                raise ValueError
            return score, pose, self.lig_pdbqt_out
...
class VinaDockingTask(BaseDockingTask):
    ...
    @classmethod
    def from_generated_mol(cls, ligand_rdmol, ligand_filename, r_ligand_filename,  protein_path_in = None, protein_root='/raid/ligl/data1/data_ge/crossdocked_v1.1_rmsd1.0_pocket10', use_uff = False, **kwargs):
        # load original pdb

        if protein_path_in is not None:
            protein_path = protein_path_in
        else:
            protein_fn = os.path.join(
        ...
        return cls(protein_path, ligand_rdmol, r_ligand_filename, use_uff = use_uff, **kwargs)

    def __init__(self, protein_path, ligand_rdmol, r_ligand_filename = None, tmp_dir= data_path_base + '/data_ge/tmp', center=None, use_uff = False,
                 size_factor=1.5, buffer=10.0): # 8
        ...
        rdDistGeom.EmbedMolecule(ligand_rdmol, 10, randomSeed=0xf00d)
        # AllChem.EmbedMolecule(ligand_rdmol, Chem.rdDistGeom.ETKDGv3())
        if use_uff:
        ...
        if r_ligand_filename!=None:
            try:
                ligand_path = r_ligand_filename
                mol_ground_truth = Chem.MolFromMolFile(ligand_path, sanitize=False)
                # mol_ground_truth = Chem.MolFromMol2File(ligand_path, sanitize=True)
                pos = mol_ground_truth.GetConformer(0).GetPositions()
            except Exception as e:
                print(e)
                rdDistGeom.EmbedMolecule(mol_ground_truth, 1000, randomSeed=0xf00d)
                pos = mol_ground_truth.GetConformer(0).GetPositions()
        if center is None:
    ...
    def run(self, mode='dock', exhaustiveness=8, save_pose=True, **kwargs):
        ...
        score, pose, lig_pdbqt_out= dock.dock(score_func='vina', mode=mode, exhaustiveness=exhaustiveness, save_pose=save_pose, **kwargs)
        return [{'affinity': score, 'pose': pose, 'lig_pdbqt_out': lig_pdbqt_out}]
