...
from gtda.homology import VietorisRipsPersistence,EuclideanCechPersistence, SparseRipsPersistence
...
def get_ph_feature(pos_o,type):
    if not isinstance(pos_o, np.ndarray):
        pos = pos_o.numpy()
    else:
        pos = pos_o
    pos = np.expand_dims(pos, axis=0)
    VR = VietorisRipsPersistence(homology_dimensions=[0, 1, 2],collapse_edges=True)  # Parameter explained in the text
    # todo: check the difference of EC and VR
    # EC = EuclideanCechPersistence(homology_dimensions=[0, 1, 2])
    diagrams = VR.fit_transform(pos)
    # diagrams2 = EC.fit_transform(pos)
    # print(diagrams.shape)
    # fig = plot_diagram(diagrams[0])
    # pio.write_image(fig, '/raid/ligl/data1/data_ge/fig/diagram/'+ str(time.time()) +type+ '.png')

    PE = PersistenceEntropy()
    features = PE.fit_transform(diagrams)
    features = features.astype('float32')
    # pos_torch = torch.squeeze(torch.from_numpy(pos),dim=0).cuda()
    # pre_ph = vr_persistence_l1(pos_torch, 1, 0)

    return torch.from_numpy(diagrams), torch.from_numpy(features)

def get_ph_sparse(pos_o,type):
    if not isinstance(pos_o, np.ndarray):
        pos = pos_o.numpy()
    else:
        pos = pos_o
    pos = np.expand_dims(pos, axis=0)
    VR = VietorisRipsPersistence(homology_dimensions=[0, 1, 2], n_jobs=6)  # Parameter explained in the text
    # todo: check the difference of EC and VR
    # EC = EuclideanCechPersistence(homology_dimensions=[0, 1, 2])
    diagrams = VR.fit_transform(pos)
    # diagrams2 = EC.fit_transform(pos)
...
class PDBProtein(object):
    ...
    def to_dict_atom(self):
        ...
        # start = time.time()
        diagrams, pos_CA_ph = get_ph_feature(pos_CA,type='feature_CA')
        # end = time.time()
        # print('pos_CA_ph', pos_CA.shape,end - start)

        # start = time.time()
        # pos_CA_ph = get_ph_sparse(pos_CA, type='sparse_CA')
        # end = time.time()
        # print('sparse_CA_ph', pos_CA.shape, end - start)
...
def parse_sdf_file(path):
    ...
    diagrams, ph = get_ph_feature(pos,type='ph')

    # end = time.time()
    # print('pos', pos.shape, end - start)
...
def parse_sdf_file_noSanitizeMol(path):
    fdefName = os.path.join(RDConfig.RDDataDir, 'BaseFeatures.fdef')
    factory = ChemicalFeatures.BuildFeatureFactory(fdefName)
    # read mol
    if path.endswith('.sdf'):
