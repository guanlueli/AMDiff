...
def drop_discrete_atom(smile):

    mol = Chem.MolFromSmiles(smile)
    frags = Chem.rdmolops.GetMolFrags(mol, asMols=True)
    droped_mol = max(frags, key=lambda x: x.GetNumAtoms())
...
def reconstruct_from_generated(xyz, atomic_nums, aromatic=None, basic_mode=True, drop = True):
    ...
    smile = Chem.MolToSmiles(rd_mol)

    if  drop == True and '.' in smile:

        rd_mol = drop_discrete_atom(smile)
